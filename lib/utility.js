...
const Big = require('../node_modules/big.js/big.min.js')
...
const utility = {
    ...
    if (_.isString(obj)) {
      ...
      if ((_match = _obj.match(/(.*)[年-](\d*)[月-](\d*)日?/))) {
        ...
        if ((_eras = _year.match(/([^\x01-\x7E\uFF61-\uFF9F]{2})(\d*)/))) {
          ...
          switch (_era) {
            case '令和':
              if (/令和\s?元/.test(_eras.input)) {
                _year = 2019
              } else {
                _year = Number(_year) + 2018
              }
              break
            case '平成':
  ...
  sma: (values, range, roundNumber) => {
    roundNumber = roundNumber || 2
    return values.map((v, i, arr) => {
      if (i < range - 1) {
        return null
      } else {
        const refArray = arr.slice(i - range + 1, i + 1)
        return parseFloat(Big(calc.mean(refArray)).round(roundNumber))
      }
    })
  }
}
